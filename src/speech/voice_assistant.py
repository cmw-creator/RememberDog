...
# API Key 设置
dashscope.api_key = os.getenv("DASHSCOPE_API_KEY", "sk-51a6a596303e466f9054520ec297af09")
...
class VoiceAssistant:
    ...
    def run_listen(self):
        ...
        try:
            while self.running and self.listening:
                ...
                recognition.send_audio_frame(enhanced)
        except Exception as e:
    ...
    def process_command(self, text):
        ...
        # 问候语
        for greeting in self.commands_db["responses"].get("greeting", []):
            if greeting in text:
        ...
        # 匹配命令
        matched = False
        for cmd in self.commands_db.get("commands", []):
            if re.search(cmd["pattern"], text):
                self.execute_action(cmd["action"], text)
                matched = True
                break

        # QA + 生成式模型
        answer, score = self.qa_manager.query(text, top_k=1, threshold=0.5)
        print(f"Q&A 匹配得分: {score:.2f}, 答案: {answer}")

        final_answer = self.call_online_model(text)
