...
class FallDetector:
    def __init__(self, camera_manager,memory_manager, min_detection_confidence=0.5, min_tracking_confidence=0.5,
        ...
        self.cooldown_period = 40  # 冷却时间
        self.cooldown_period_now = 0  # 冷却时间
    ...
    def run(self, source=0):
        ...
        while True:
            ...
            if frame is None:
                ...
                continue
            frame_id += 1
            self.cooldown_period_now-=1
            if self.cooldown_period_now<=0:
                self.cooldown_period_now=0
            ...
            if results.pose_landmarks:
                ...
                if status == "跌倒" and self.cooldown_period_now<=1:
                    self.cooldown_period_now+=self.cooldown_period
                    event_payload = {
                    "speak_text": "您好像跌倒了，需要我帮您联系您的家人吗？",
                    "timestamp": time.time(),
                    "audio_file": "assets/voice/dog/20.wav"
                    }
        
                    self.memory_manager.trigger_event("speak_event", event_payload)
                    print("发送信息到memory_manager")
...
if __name__ == "__main__":
    ...
    camera_manager = CameraManager("rtsp://192.168.1.120:8554/test")
    memory_manager = MemoryManager()
    speech_engine = SpeechEngine(memory_manager)
    detector = FallDetector(camera_manager,memory_manager)
